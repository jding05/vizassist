...
//    }
    ...
    public void askForPermission(String permission, Integer requestCode) {
        if (ContextCompat.checkSelfPermission(activity, permission) != PackageManager.PERMISSION_GRANTED) {
            ...
            // Should we show an explanition? the user see the permission before, but didn't accept, so pop up again
            if (ActivityCompat.shouldShowRequestPermissionRationale(activity, permission)) {
                // this is called if user has denied the permission before
                // in this case, I am just asking the permission again
                ActivityCompat.requestPermissions(activity, new String[]{permission}, requestCode);
                // we can add the note for user that this app is camera required for feature used
            } else {
                // this activity is our mainActivity
                ActivityCompat.requestPermissions(activity, new String[]{permission}, requestCode);
            }
        }
    }
